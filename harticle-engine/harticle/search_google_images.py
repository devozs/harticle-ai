...
def scrape_and_download_google_images(query):
    queryParameters = {"q": query, "tbm": "isch", "hl": "en", "gl": "us", "ijn": "0"}
    html = requests.get("https://www.google.com/search", params=queryParameters, headers=headers, timeout=30)
    soup = BeautifulSoup(html.text, "lxml")

    images_data_in_json = convert_image_to_json(soup)
    ...
    for index, (image_data, image_link) in enumerate(zip(soup.select(target_image_path), full_resolution_images),
                                                     start=1):

        if "http:" in image_link or "facebook" in image_link:
            print(f'skipping image {image_link}')
            continue
...
def convert_image_to_json(soup):
...
def append_image_to_list(image_data, image_link):
    html_path = image_data.select_one(targetted_image_base_html_path)
    title = "missing"
    source_link = "missing"
    if html_path is None:
        ...
        title = html_path["title"]
        source_link = html_path["href"]

    google_images.append({
        "image_title": title,
        "image_source_link": source_link,
...
def get_google_image(query):
    scrape_and_download_google_images(query)
    if len(google_images) > 0:
        selected = random.choice(google_images)
        selected_link = selected['image_link']
        print(selected_link)
